...
if(policy>dict*1.5) return 'policy'
if(dict>policy*1.5) return 'dictionary'
return 'mixed'
}
...
if(!h.metadata?.measure) return false
...
if(!hasSubjectInTopChunk&&topScore<MIN_RELEVANCE_SCORE*2) return false
return topScore>=MIN_RELEVANCE_SCORE
}
...
}else if(intent==='definition'&&docType!=='policy'){
...
if(!text?.trim()) return []
if(isPolicyDocument(text,fileName)) return chunkPlainText(text,fileName,POLICY_CHUNK_SIZE,POLICY_CHUNK_OVERLAP,true)
...
return sorted.map(item=>({measure:item.measure,table:item.table,shortDescription:item.shortDescription,formula:item.formula,confidenceScore:Math.min(100,Math.round((item.score/maxScore)*100))}))
}
let db=null
